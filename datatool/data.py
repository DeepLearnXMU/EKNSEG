...
class GraphField(data.Field):
    def preprocess(self, x):
        entities,ent_com=x
        return (entities.strip(),ent_com.strip())
...
class DocDataset(data.Dataset):
    def __init__(self, path, text_field, order_field, graph_field,
        ...
        examples = []
        path, einspath, ecommonpath = path
        with open(path, 'r', encoding=encoding) as f, open(einspath, 'r') as fe,open(ecommonpath,'r',encoding=encoding)as fec:
            for line, lineeins,lineecommon in zip(f, fe,fec):
                line = line.strip()
                order = list(range(line.count('<eos>') + 1))
                if 'train' in path:
                    if len(order) > 1:
                        examples.append(data.Example.fromlist([line, order, (lineeins,lineecommon)], fields))
                else:
                    examples.append(data.Example.fromlist([line, order, (lineeins,lineecommon)], fields))
...
class MyBatch(Batch):
    def __init__(self, allsentences, orders, doc_len, ewords, elocs,ent_rel_list, dataset=None,
                 device=None):
        """Create a Batch from a list of examples."""
        if data is not None:
            setattr(self, 'doc_len', doc_len)
            setattr(self, 'elocs', elocs)
            setattr(self, 'ent_rel_list',ent_rel_list)
    ...
    def __iter__(self):
        while True:
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                for ex in minibatch:
                    sfdoc, sforder = ex.doc, ex.order
                    
                    randid = list(range(len(sforder)))
                    shuffle(randid)

                    sfdoc = [sfdoc[ri] for ri in randid]

                    sforder = [sforder[ri] for ri in randid]
                    ...
                    ewset = set(ew)
                    for entity_relation in entity_relations.split():
                        if len(entity_relation) <= 0 or len(entity_relation.split('&')) > 2:
                            continue
                        entity1, entity2 = entity_relation.split('&')
                        entity2, score = entity2.split('#')
                        if entity1 in ewset and entity2 in ewset:
                            ent_rel_of_sent.append([ew.index(entity1), ew.index(entity2),float(score)])

                    elocs.append(newlocs)
                    ewords.append(ew)
                    entity_relation_list.append(ent_rel_of_sent)

                yield MyBatch(allsentences, orders, doc_len, ewords, elocs,entity_relation_list,
