...
from datatool.data import DocField, DocDataset, DocIter, GraphField
import time
...
def parse_args():
    ...
    # dataset settings
    parser.add_argument('--corpus', type=str, nargs='+',default=('aan/train.lower','aan/train.eg.20.nosingle','aan/train.eg.20.nosingle.common'))
    parser.add_argument('--lang', type=str, nargs='+', help="the suffix of the corpus, translation language")
    parser.add_argument('--valid', type=str, nargs='+',default=('aan/val.lower','aan/val.eg.20.nosingle','aan/val.eg.20.nosingle.common'))

    parser.add_argument('--writetrans', type=str, help='write translations for to a file',default='decoding/ann_0.5_gdp_0.3_gl3.devorder')
    parser.add_argument('--ref', type=str, help='references, word unit')

    parser.add_argument('--vocab', type=str,default='aan/vocab.new.100d.lower.pt')
    ...
    parser.add_argument('--d_rnn', type=int, default=500, help='dimention size for FFN')
    parser.add_argument('--d_mlp', type=int, default=500, help='dimention size for FFN')
    parser.add_argument('--senenc', default='bow', help='sentence encoder')

    parser.add_argument('--gnnl', default=3, type=int, help='stacked layer number')
    parser.add_argument('--gnndp', default=0.3, type=float, help='self-att dropout')
    
    parser.add_argument('--labeldim', default=50, type=int, help='label dim')
    parser.add_argument('--agg', choices=['gate', 'att'], help='node agg method',default='gate')

    parser.add_argument('--reglamb', default=0, type=float)
    parser.add_argument('--loss', default=0, type=int)

    parser.add_argument('--entityemb', choices=['glove', 'lstm'],default='glove')
    ...
    parser.add_argument('--test', type=str, nargs='+', help='test src file',default=('aan/test.lower','aan/test.eg.20.nosingle','aan/test.eg.20.nosingle.common'))
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        if args.resume:
            ...
            except_name = ['mode', 'resume', 'maximum_steps', 'offset']
